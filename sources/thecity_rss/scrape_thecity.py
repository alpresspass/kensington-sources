#!/usr/bin/env python3
"""
Scrape The City.nyc RSS feed for NYC news.
The City covers NYC-focused news including transportation, city government, and local events.
"""
...
# Add project root to path for imports (sources/thecity_rss/ -> sources/ -> kensington-sources/)
sys.path.insert(0, str(Path(__file__).parent.parent.parent))
from models import RSSItem

SOURCE_NAME = "thecity_rss"
BASE_URL = "https://www.thecity.nyc/feed/"
OUTPUT_DIR = Path(__file__).parent / "scrape_items"
LOG_FILE = Path(__file__).parent / "scrape_log.txt"

def parse_args():
    parser = argparse.ArgumentParser(description="Scrape The City.nyc RSS feed")
...
def scrape_thecity(start: datetime, end: datetime) -> list[RSSItem]:
    """
    Scrape The City.nyc RSS feed for items within the date range.
    Returns a list of RSSItem objects.
    """
    log_message(f"Starting The City scrape from {start} to {end}")
    ...
    log_message(f"Found {len(items)} The City items")
    return items
...
def main():
    ...
    items = scrape_thecity(start, end)
    save_items(items)
    
    log_message("The City scrape complete")
