#!/usr/bin/env python3
"""
Scrape BKMAG RSS feed for Brooklyn news.
BKMAG covers NYC/Brooklyn local news, music, bars, restaurants, books, sex, real estate and events.
"""
...
# Add project root to path for imports (sources/bkmag_rss/ -> sources/ -> kensington-sources/)
sys.path.insert(0, str(Path(__file__).parent.parent.parent))
from models import RSSItem

SOURCE_NAME = "bkmag_rss"
BASE_URL = "https://www.bkmag.com/feed/"
OUTPUT_DIR = Path(__file__).parent / "scrape_items"
LOG_FILE = Path(__file__).parent / "scrape_log.txt"

def parse_args():
    parser = argparse.ArgumentParser(description="Scrape BKMAG RSS feed")
...
def scrape_bkmag(start: datetime, end: datetime) -> list[RSSItem]:
    """
    Scrape BKMAG RSS feed for items within the date range.
    Returns a list of RSSItem objects.
    """
    log_message(f"Starting BKMAG scrape from {start} to {end}")
    ...
    log_message(f"Found {len(items)} BKMAG items")
    return items
...
def main():
    ...
    items = scrape_bkmag(start, end)
    save_items(items)
    
    log_message("BKMAG scrape complete")
